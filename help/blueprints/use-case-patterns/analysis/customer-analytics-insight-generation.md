...
- Les droits du produit CJA sont configurés pour l’organisation
- Les profils de produit CJA sont configurés avec un accès utilisateur approprié (création d’espace de travail, accès aux vues de données)
- Le sandbox AEP contient les jeux de données cibles avec un flux de données (événements web, événements d’application, données de campagne, enregistrements CRM)
- Les schémas XDM sont définis pour tous les jeux de données sources avec les groupes de champs appropriés
- Le champ ID de personne est identifié et systématiquement disponible dans tous les jeux de données qui seront connectés
...
- Pour l’option C (Publication d’audiences) : le profil client en temps réel AEP est activé dans le sandbox cible
...
- **Limites de connexion :** le nombre maximal de connexions par organisation est limité par les droits de SKU CJA. Une seule connexion peut inclure des jeux de données provenant d’un seul sandbox AEP. — [Mécanismes de sécurisation de CJA](https://experienceleague.adobe.com/fr/docs/analytics-platform/using/cja-admin/guardrails)
