...
Pour établir une connexion API sécurisée à un produit Adobe, Adobe fournit la création d’informations d’identification de serveur à serveur OAuth. Pour ce faire, vous devez d’abord créer un projet de développement dans le Adobe Developer Console. Pour pouvoir accéder au Developer Console, des droits de développeur doivent vous avoir été attribués dans le Adobe Admin Console. Une fois que vous disposez de ces droits, vous pouvez créer des projets de développement à l’aide des différentes API liées au produit Adobe. C’est là que les informations d’identification de serveur à serveur OAuth entrent en jeu. Pour générer un jeton d’accès, vous devez transmettre un certain ensemble de revendications au service Identity Management (IMS) d’Adobe. Pour les informations d’identification de serveur à serveur OAuth, un exemple d’appel se présenterait comme suit :
...
>[!CAUTION]
>
>Si vous ne spécifiez pas le paramètre `x-sandbox-name`, la requête échoue normalement. Par défaut, la requête est traitée dans le sandbox `default` automatiquement configuré avec n’importe quel environnement Experience Platform

>[!NOTE]
>
>Dans le cadre de ce bootcamp, nous avons créé un projet de développement et vous avons fourni un fichier d’environnement Postman avec toutes les valeurs nécessaires pour demander une `access_token`. Voici ce que vous avez chargé lors des étapes précédentes du Lab.
...
`token_type` - sera toujours de type porteur
...
Cela se produit lorsque le `private_key` de votre fichier d’environnement est malformé ou n’est plus valide. Si vous voyez ceci, assurez-vous d’avoir copié la clé entière, y compris les sauts de ligne
